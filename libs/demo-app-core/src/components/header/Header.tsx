...
export const CoreHeader: React.FC = () => {
  ...
  return (
    <Navigation.Root>
      <Select.Root onValueChange={handleChange} defaultValue={currentLanguage} className="md:mb-0 mb-4">
        <Select.Item value={AppLang.EN}>{t('lang.en')}</Select.Item>
        <Select.Item value={AppLang.AR}>{t('lang.ar')}</Select.Item>
      </Select.Root>
      <Navigation.Item link={fromNav.DASHBOARD}>{t('navigation.home')}</Navigation.Item>
      <Navigation.Item link={fromNav.USERS}>{t('navigation.users')}</Navigation.Item>
      <Navigation.Item link={fromNav.SIGN_UP}>{t('navigation.form')}</Navigation.Item>
      <Navigation.Item link={fromNav.SOMETHING}>{t('navigation.something')}</Navigation.Item>
    </Navigation.Root>
  )
}
